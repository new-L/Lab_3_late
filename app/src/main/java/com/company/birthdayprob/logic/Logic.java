...
public class Logic 
    ...
    */
    ...
    public double calculate(int size, int count) {
        ...
        int dupl = 0;// количество прогонов(людей), которые имеют два дня рождения в тот же день
        for (int i = 0; i < count; i++) {
            int day[] = new int[365];// создаем массив, соответствующий каждому дню года
            random.setSeed(i+1);//начальное значение для объекта Random, с номером пробега i+1(т.к. нулевого дня не существует)
            for (int j = 0; j <size ; j++) {
                int n = random.nextInt(365);//генераия рандомного целочисленного числа
                day[n]++;//(т.к. нулевого дня не существует)
                if(day[n] >= 2) {// у двух людей одинаковый день рождения
                    dupl++;
                    break;
                }
            }
        }
        return dupl * 100.0 / count;// возвращаем процент, у которых два человека с одинаковым днем рождения
    }
